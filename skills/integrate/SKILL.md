...
swept in that no agent wrote, closed on the human's decision at merge: `- [x] false positive` where the
file belonged in the commit, or `- [x] won't fix — <reason>` where it did not.
